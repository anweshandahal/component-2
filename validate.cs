...
namespace Anweshdahal
{
    ...
    public class Validation
    {
        ...
        /// <summary>
        /// boolean value for validaiton
        /// </summary>
        public Boolean isValidCommand = true, isSomethingInvalid = false;
        /// <summary>
        /// setting variable and other factors
        /// </summary>
        public int Raduis = 0, Width = 0, Height = 0, count = 0, LoopCounter = 0, lineNumber = 0;
        /// <summary>
        /// for pen on/pff and pen color
        /// </summary>
        public int fill, on, off, pen, green, yellow, pink, red;
        /// <summary>
        /// boolean for the purpose of checking loop
        /// </summary>
        public Boolean hasLoop = false, hasEndLoop = false;
        // public Boolean hasonlyIf = false;
        /// <summary>
        /// for the purpose of if end if check
        /// </summary>
        public Boolean hasIf = false, hasEndif = false;
        /// <summary>
        /// for checking loop line
        /// </summary>
        ...
        /// <summary>
        /// to check the command line for validation
        /// </summary>
        /// <param name="lineOfCommand"></param>
        public void checkLineValidation(string lineOfCommand)
        {
            String[] keyword = { "circle", "arc", "rectangle", "triangle", "square", "rotate", "pen", "green", "yellow", "pink", "red", "fill", "on", "off", "drawto", "moveto", "repeat", "if", "endif", "loop", "endloop" };
            String[] shapes = { "circle", "arc", "rectangle", "triangle", "sqaure" };
            ...
            if (firstWordIsKeyword)
            {
                ...
                if (firstWordIsShape)
                {
                    ...
                    else if (words[0].ToLower().Equals("arc"))
                    {
                        /* String args = lineOfCommand.Substring(10, (lineOfCommand.Length - 10));
                         String[] parms = args.Split(',');
                    ...
                    if (words[0].ToLower().Equals("square"))
                    {
                        if (words.Length == 2)
                        {
                            Boolean isInt = words[1].All(char.IsDigit);
                ...
                else if (firstWord.Equals("loop"))
                {
                    if (words.Length == 2)
                    {
                        if (words[1].ToLower().Equals("count"))
                        {
                        }
                        else
                        {
                        ...
                        if (variable.Contains(words[1].ToLower()))
                        {
                            if (words[2].Equals("="))
                            {
                                ...
                                if (isInt)
                                {
                                    if (words[4].ToLower().Equals("then"))
                                    {
                                        if (shapes.Contains(words[5].ToLower()))
                                        {
                                            // isValidCommand = true;
                                        }
